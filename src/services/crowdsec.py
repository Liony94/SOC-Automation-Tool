...
class CrowdSecService:
    def __init__(self):
        ...
        self.enabled = bool(self.api_key)  # Service activé seulement si une clé API est fournie
        self.headers = {
            "X-Api-Key": self.api_key,
            "Content-Type": "application/json"
        }

    async def add_decision(self, ip: str, duration: str = "4h", reason: str = "Malicious activity detected") -> Dict:
        """Ajoute une décision de blocage dans CrowdSec"""
        if not self.enabled:
            logger.warning("CrowdSec n'est pas configuré, ignoré l'ajout de décision")
            return {"status": "skipped", "reason": "CrowdSec non configuré"}
    ...
    async def check_ip(self, ip: str) -> Optional[Dict]:
        ...
        if not self.enabled:
            logger.warning("CrowdSec n'est pas configuré, ignoré la vérification")
            return None
        
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.base_url}/decisions/stream?ip={ip}",
                    headers=self.headers
                ) as response:
